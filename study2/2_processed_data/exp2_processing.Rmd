...
```{r}
file_names <- list.files("../1_raw_data/", pattern=".csv") #where you have your files

child_data <- data.frame()

subjs <- read_csv("../child_subj_info.csv")
...
child_data <- dplyr::rename(child_data, feedback=Correction, response=Reward, card = Card, guess = Guess)
...
child_data_final <- select(child_data, -Reaction, -Birthday, -Date)
